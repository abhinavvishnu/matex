...
size_t find_num_samples(ifstream &in) {
    ...
    while (in.good()){
        string line;
        getline(in, line);
        size_t ntokens = Tokenize(line, junk, tmp_samples);
...
bool Dataset::readsparse(Comm *comm, size_t offset ,size_t &max_item_id) {
    ...
    if (comm->rank() == 0) {
        ...
        while (inp_file.good()) {
            ...
            size_t ntokens = Tokenize(line, max_value, samples);

            if (max_value > max_item_id)
