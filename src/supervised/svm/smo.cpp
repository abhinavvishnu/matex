...
SMO::SMO(int *argc, char ***argv) {
    ...
    if (comm->rank() == 0) {
        cout << "MaTEx Support Vector Machine Algorithm:" << endl;
    }
...
void SMO::updategradient(size_t i1, size_t i2, double a1, double a2, double delalph1,
    ...
    for (size_t i = 0; i < trainset->row_ptr.size(); i++) {
       ...
       {
           if ( trainset->samples[offset + nelems_sample - 2] < upmin)
           {
               upmin =  trainset->samples[offset + nelems_sample - 2];
               b_up = upmin;
               i_up = global_index;
           }
       }
       ...
       {
           if (trainset->samples[offset + nelems_sample - 2] > lowmax)
           {
               lowmax = trainset->samples[offset + nelems_sample - 2];
               b_low = lowmax;
               i_low = global_index;
           }
       }
    ...
    if (iter_count++ % 500 == 0 && 0 == comm->rank())
        cout << "iter: " << iter_count << ", b_up: " << b_up <<", b_low: "<< b_low << endl;
...
double SMO::addFcache() {
    ...
    for (size_t i = 0; i < trainset->row_ptr.size(); i++) {
        ...
        if (alphaval > 1e-12 && fabs(alphaval - C) > 1e-12) {
...
void SMO::run() {
    ...
    // initialize the data structures for testing
    testing_init();
    testing(thresh);
