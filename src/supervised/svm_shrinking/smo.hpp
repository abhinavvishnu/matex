...
class SMO {
    ...
    public:
        ...
        vector<size_t> working_set; // holds the indices of active samples
        size_t n_working_set; // holds the number of active samples
        size_t shrinkiter, shrinkitercounter;
        ...
        void reconstruct_gradient_serial();
        void reset_shrink_ds();

        bool shrink_sample(double, double, double, double, size_t);
        void shrinking_init();
        int exchange_samples_and_row_ptr(int index);
        void write_gradient_and_update_ds();

        void find_global_bup_blow();
        void update_shrinking_stats();

        SMO();
        SMO(int*, char***);
        void testing_init();
        int exchange_test_samples_and_row_ptr(int index);
        void testing(double);
